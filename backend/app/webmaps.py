...
# Originally from gis-administration by Joshua Sharp Heward (https://github.com/joshsharpheward/gis-administration)
# Modified by David C Jantz in 2025
# Licensed under GPLv3 (https://www.gnu.org/licenses/gpl-3.0.en.html)
def get_unused(instance_item, username=None, password=None):
    """
    Identify and return unused services (Map, Feature, Vector Tile, and Image Services) in an ArcGIS Portal.

    This function connects to an ArcGIS Portal instance and searches for all relevant services,
    excluding those owned by Esri. It then checks all web maps and identifies services used in
    basemap layers or web map layers, filtering them from the unused list.

    :param instance_item: The ArcGIS Portal instance to connect to.
    ...
    :returns: A list of unused services (Map, Feature, Vector Tile, Image Services).
    :rtype: list

    :note:
        The function searches for up to 10,000 services per type and excludes Esri-owned services.
    """
    ...
    try:
        ...
        # Fetch services (excluding Esri-owned ones) in a more structured way
        service_types = ["Map Service", "Feature Service", "Vector Tile Service", "Image Service"]
        services = {service.url: service for stype in service_types
                    for service in target.content.search(query="NOT owner:esri*", item_type=stype, max_items=10000)}

        # Fetch all Web Maps in the portal
        web_maps = target.content.search(query="NOT owner:esri*", item_type="Web Map", max_items=10000)

        # Identify used services
        used_services = set()

        for item in web_maps:
            web_map = WebMap(item)

            # Extract basemap layers and regular layers
            basemap_layers = web_map.basemap.get("baseMapLayers", [])
            map_layers = web_map.layers

            # Collect URLs from all layers
            for layer in basemap_layers + map_layers:
                for key in ["styleUrl", "url"]:
                    if key in layer and layer[key] in services:
                        used_services.add(layer[key])

        # Determine unused services
        unused_services = [services[url] for url in services.keys() - used_services]

        logger.debug(f"Total unused services found: {len(unused_services)}")

        return {"unused_services": unused_services}
    except Exception as e:
        logger.exception(f"Error retrieving unused services for {instance_item}: {e}")
        return {"error": f"Error retrieving unused services for {instance_item}"}
